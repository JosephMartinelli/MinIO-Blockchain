...
@app.post("/register-transactions",
          status_code=200)
...
async def register_node(client: Client):
    ...
    if not client:
        return JSONResponse(status_code=404,content="Node address cannot be an empty field!")
    if not peers.intersection(f"{client['ip_address']}:{client['port']}"):
        peers.add(f"{client['ip_address']}:{client['port']}")
    else:
        return JSONResponse(status_code=400,content="Client already present into peers!")
    chain =  await get_chain()
    return {"chain":chain,"peers":list(peers)}

@app.post(path="/register-with-node",
         status_code=200)
async def register_with_node(node_to_regiser: RegisterNode,request: Request):
    ...
    # Register with node
    try:
        response = requests.post(url=f"http://{node_info['node_address']}:{node_info['node_port']}/register-node",
                                 data=json.dumps({"ip_address":request.client.host,"port":port}))
    except requests.exceptions.ConnectionError:
        return JSONResponse(status_code=400,content=f"Could not connect with node at {node_info['node_address']}")
    if response.status_code !=201:
        return JSONResponse(status_code=400,content=f"Could not register with node at {node_info['node_address']}\n, {response.content}")
    # If I could register, then I update my local view of the blockchain
    global blockchain,peers
    data = response.json()
    peers.update(data['peers'])
    return JSONResponse(status_code=200,content=f"Successfully registered to node {node_info['node_address']}, and now I can see the following"
                                                f"peers: {peers}")
...
async def add_block(in_block: InputBlock):
    ...
    try:
        ...
        return JSONResponse(status_code=400,content=f"Block discarded by the node due to the following error: {e}")
    if not blockchain.is_chain_valid():
        blockchain.chain.pop(-1)
        return JSONResponse(status_code=400,content=f"Last block invalidated the chain, reverting back...")
...
def announce_new_block():
    ...
    for peer_url in peers:
        response = requests.post(url=f"http://{peer_url}/add-block",data=json.dumps(block_to_announce.__dict__))
        if response.status_code != 201:
            raise RuntimeError(f"Error in announcing the new block to the other peers\n {response.content}")
